...
extension InternetAddress {
    ...
    private static func getAddressFromHostname(hostname: String) throws -> in_addr {
        ...
        guard let hostInfo = gethostbyname(hostname)?.pointee else {
        ...
        guard let h_addr_list = hostInfo.h_addr_list else {
            throw Error(.FailedToGetIPFromHostname("List is empty"))
        }
        
        let addrStruct = sockadd_list_cast(p: h_addr_list)[0].pointee
        return addrStruct
    }
}
